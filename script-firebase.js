...
// Función para validar datos antes de cargar
window.validateBulkData = function() {
    ...
    updateBulkStats(validCount, errorCount, duplicateCount);
    
    const importStatus = document.getElementById('import-massive-status');
    if (importStatus) {
        importStatus.style.display = 'block';
        importStatus.className = `import-status ${errorCount > 0 ? 'warning' : 'success'}`;
        importStatus.textContent = `Validación completada: ${validCount} válidos, ${errorCount} errores`;
    }
};
            ...
            try {
                ...
                if (pasteTableBody) {
                    ...
                    if (importStatus) {
                        importStatus.style.display = 'block';
                        importStatus.className = 'import-status success';
                        importStatus.textContent = `Archivo cargado exitosamente: ${jsonData.length - 1} registros`;
                    }
                }
            } catch (error) {
...
// Función mejorada para importar tabla pegada
window.importPasteTable = async function() {
    const pasteTableBody = document.getElementById('paste-table-body');
    const importStatus = document.getElementById('import-massive-status');
    ...
    if (validCount > 0) {
        const confirmMessage = `¿Estás seguro de que quieres cargar ${validCount} registros válidos a Firebase?\n\nEsta acción no se puede deshacer.`;
        if (!confirm(confirmMessage)) {
            return;
        }
    } else {
        alert('No hay registros válidos para cargar. Por favor, valida los datos primero.');
        return;
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        progressText.textContent = `Procesando registro ${i + 1} de ${totalRows}`;
        
        // Validación mejorada
        if (!ubch || !community || !name || !cedula || !sexo || !edad) {
            errors++;
            tr.style.background = '#fff3cd';
            statusCell.innerHTML = '<span style="color: #856404;">❌ Campos faltantes</span>';
            continue;
        }
        if (!/^\d{6,10}$/.test(cedula)) {
            errors++;
            tr.style.background = '#fff3cd';
            statusCell.innerHTML = '<span style="color: #856404;">❌ Cédula inválida</span>';
            continue;
        }
        if (telefono && !/^04\d{9}$/.test(telefono)) {
            errors++;
            tr.style.background = '#fff3cd';
            statusCell.innerHTML = '<span style="color: #856404;">❌ Teléfono inválido</span>';
            continue;
        }
        if (!['M','F','m','f'].includes(sexo)) {
            errors++;
            tr.style.background = '#fff3cd';
            statusCell.innerHTML = '<span style="color: #856404;">❌ Sexo inválido</span>';
            continue;
        }
        if (isNaN(edad) || edad < 16 || edad > 120) {
            errors++;
            tr.style.background = '#fff3cd';
            statusCell.innerHTML = '<span style="color: #856404;">❌ Edad inválida</span>';
            continue;
        }
        ...
        try {
            ...
            if (!existing.empty) {
                ...
                statusCell.innerHTML = '<span style="color: #856404;">🔄 Duplicado</span>';
                continue;
            }
            ...
            statusCell.innerHTML = '<span style="color: #28a745;">✅ Cargado</span>';
        } catch (error) {
            errors++;
            tr.style.background = '#f8d7da';
            statusCell.innerHTML = '<span style="color: #dc3545;">❌ Error</span>';
            console.error('Error guardando registro:', error);
        }
    }
    ...
    importStatus.className = `import-status ${errors > 0 ? 'warning' : 'success'}`;
    importStatus.textContent = `Proceso completado: ${count} registros cargados, ${errors} errores, ${duplicates} duplicados`;
...
class VotingSystemFirebase extends VotingSystem {
    ...
    renderRegistratorHistory() {
        ...
        // Renderizar tabla
        this.renderRegistratorHistoryTable(filteredRegistrations);
    ...
    renderRegistratorHistoryTable(registrations) {
    ...
    goToRegistratorPage(page) {
        ...
        console.log(`Navegando a página ${page} del historial del registrador`);
    }
